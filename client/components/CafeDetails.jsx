...
const CafeDetailsScreen = ({ route }) => {
  ...
  const { menuIds, name, cafeteriaId, cafeteriaImage, rating, openStatus } =
    route.params;
    ...
    const fetchFoodItems = async () => {
      try {
        const responses = await Promise.all(
          menuIds.map((id) => API.get(`/foods/${id}`))
        );
        const fetchedItems = responses.map((res) => res.data);
        console.log("Fetched food items:", fetchedItems);

        // Filter out null or invalid data
        const validItems = fetchedItems.filter(
          (item) => item && item.foodImage && item.foodName
        );
        const invalidItems = fetchedItems.filter(
          (item) => !item || !item.foodImage
        );
        if (invalidItems.length > 0) {
          console.warn("Invalid food items:", invalidItems);
        }
  ...
  const handleAddToCart = async (food, quantity) => {
    ...
    const proceedWithAdd = async () => {
      try {
        ...
        if (
          errorMsg === "Cart already contains items from a different cafeteria"
        ) {
          Alert.alert(
            "Different Cafeteria",
            ...
            [
              { text: "Cancel", style: "cancel" },
              {
                ...
                onPress: async () => {
                  try {
                    await API.put(
                      `/reservations/${activeReservation._id}/cancel`
                    );
                    setActiveReservation(null);
